...
import tk.yimiao.yimiaocloud.microservice.mall.base.dto.EsCount;
import tk.yimiao.yimiaocloud.microservice.mall.base.dto.EsInfo;
...
//    @Value("${ES_CONNECT_IP}")
    ...
    public int importAllItems() {
        ...
        try {
            ...
            e.printStackTrace();
            throw new BusinessException("导入ES索引库出错，请再次尝试");
        }
    ...
    public EsInfo getEsInfo() {
        ...
        if (StringUtils.isBlank(healthResult) || StringUtils.isBlank(countResult)) {
            throw new BusinessException("连接集群失败，请检查ES运行状态");
        }
        EsInfo esInfo = new EsInfo();
        EsCount esCount = new EsCount();
        try {
            esInfo = JSON.parseObject(healthResult, EsInfo.class);
            esCount = JSON.parseObject(countResult, EsCount.class);
            esInfo.setCount(esCount.getCount());
        } catch (Exception e) {
            e.printStackTrace();
            throw new BusinessException("获取ES信息出错");
        }
